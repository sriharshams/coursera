---
title: "Exponential distribution in R VS Central Limit Theorem (CLT)"
...
simMeans <- NULL
for(i in 1:1000) {
  simMeans <- c(simMeans, mean(rexp(40, 0.2)))
}
mean(simMeans)

```

$\bar{x}$ in our case is `r round(mean(simMeans), 2)` which is very close to the mean of the theoretical distribution namely $\mu = \frac{1}{0.2} = 5$.
...
$s^{2}$ in our case is `r round(var(simMeans), 2)` which is close to the variance of the theoretical distribution we mentioned above.
...
data <- as.data.frame(simMeans)
ggplot(data, aes(x = simMeans)) + 
  geom_histogram(binwidth = 0.2, color = 'black', fill="#0072B2", aes(y = ..density..)) +
